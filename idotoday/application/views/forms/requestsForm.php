...
if($renderAskBtn){
  ...
  echo '<div id="rsq_frm" style="display: none;">
  <div class="row justify-content-center mt-3">
  <h2 class="font-bold">Ask for help</h2>
  </div>
  ...
  echo '</div>';
  // echo '<button type="button" id="askForHbtn" onclick="askForHelp()">Ask for help</button>';
 echo form_open('user/requests/request','id="request" autocomplete="on"');
...
<input type="text" name="token" hidden value="<?php if(isset($_SESSION["request_token"]))echo $_SESSION["request_token"];?>">


<input type="text" name="request_id" value="" hidden>

<div class="form-group row">
      <label for="toWhom" class="col-sm-3 col-form-label">To Whom</label>
    <div class="col-sm-9">
 <span id="req_fo_to_who">
 <?php if(isset($user['user_id'])){
   echo '<input class="form-control" type="text" id="toWhom" name="toWhomUserName" value="'.$user["firstName"].' '.$user["lastName"].'" readonly>
   <input type="text" name="toWhom" value="'.$user["user_id"].'" readonly hidden>';

   
 }else{
   echo '<input class="form-control" type="text" name="toWhom" id="toWhom" value="All Around You" readonly>';
...
<input class="form-control" type="text" name="request" placeholder="Request" value="<?php if (isset($_POST['request']))echo $_POST['request']; ?>" required>
</div>

<div class="form-group">
<textarea class="form-control" name="description" class placeholder="Description Or Tasks it involves. (Optional)" value="<?php if(isset($description)) echo $description;?>" rows="4"></textarea>
</div>


<div class="form-group">
<input class="form-control" type="text" name="place" placeholder="Place" value="<?php if (isset($_POST['place']))echo $_POST['place']; ?>" required>
</div>
...
<input class="form-control" type="text" name="bounty" list="bounty" placeholder="Bounty Or Reward" value="<?php if (isset($_POST['bounty']))echo $_POST['bounty']; ?>">
</div>

<div class="col-4">
<div class="form-group">
<select class="form-control" name="currency">
<option selected value="America (United States) Dollars - USD">America (United States) Dollars – USD</option>
...
<input class="btn px-5 btn-info" type='submit' name='submit' id='submitRequest' value='Request'>
</div>

<div class="form-group row justify-content-center">

<input class="btn px-5 btn-info" type="submit" name="submit" id="updateRequest" value="Update" style="display: none;">
<button class="btn-link px-5 btn-info" type="button" id="newRequest" onclick="newRequestbtn()" style="display: none;">New Request</button>
</div>
